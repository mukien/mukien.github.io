(() => {
  ...
  const led = $('led'), statusEl = $('status'), noteEl = $('note'), mirrorEl = $('mirror'), bpmEl = $('bpm');

  let audioCtx, masterGain, recDest;
  let mediaRecorder, recChunks = [], isRec = false;
  let recCtx, combined, lastBlob, isMirror = false;
  ...
  function playTone(note, vel) {
    ...
    if (old) {
      try {
        ...
        old.gain.gain.setValueAtTime(0, audioCtx.currentTime);
      } catch(e) {}
      setTimeout(() => { try { old.osc.stop(); old.gain.disconnect(); } catch(e) {} }, 50);
  ...
  function stopAllTones() {
    activeOscs.forEach(v => { try { v.osc.stop(); v.gain.disconnect(); } catch(e) {} });
    activeOscs.clear();
  }
  ...
  async function startRec() {
    if (isRec) return;
    if (!v.src || v.src === location.href) { alert('请先加载视频'); return false; }
    initAudio();
    setupCanvas();
    ...
    if (!mt) { alert('不支持录制'); return false; }
    recChunks = [];
  ...
  function updateRecUI(rec) {
    ...
    btn.innerHTML = rec
      ...
      : '<span class="material-symbols-outlined">fiber_manual_record</span> 录制';
  ...
  function rebuild() {
    ...
    const firstNote = all.find(e => e.type === 'noteOn');
    firstNoteTimeMs = firstNote ? getTickMs(firstNote.tickTime) : 0;

    playerEvents = [];
    let hasFirstNote = false;
    let prevTick = firstNote ? firstNote.tickTime : 0;

    all.forEach(e => {
      if (!hasFirstNote && e.type === 'noteOn') {
        playerEvents.push({ ...e, delta: 0 });
        hasFirstNote = true;
        prevTick = e.tickTime;
      } else if (hasFirstNote) {
        const delta = Math.max(1, getTickMs(e.tickTime - prevTick));
        playerEvents.push({ ...e, delta });
        prevTick = e.tickTime;
      }
    });

    totalDur = all.length ? getTickMs(all[all.length - 1].tickTime - (firstNote ? firstNote.tickTime : 0)) + 3000 : 0;
  }
  ...
  async function loadMidi(file) {
    ...
    try {
      ...
      midiBPM = result.bpm;
      midiTPB = result.ticksPerBeat;
      bpmEl.textContent = midiBPM;

      if (!parsedTracks.length) {
        alert('MIDI文件中没有找到音符事件');
        return;
      }
  ...
  function parseMidi(data) {
    ...
    if (String.fromCharCode(...data.slice(pos, pos + 4)) !== 'MThd') throw new Error('无效MIDI');
    pos += 4;
    ...
    while (pos < data.length - 8) {
      const ct = String.fromCharCode(...data.slice(pos, pos + 4)); pos += 4;
      const cl = (data[pos] << 24) | (data[pos + 1] << 16) | (data[pos + 2] << 8) | data[pos + 3]; pos += 4;
      if (ct !== 'MTrk') { pos += cl; continue; }
      const te = pos + cl;
      let rs = null, events = [], tickTime = 0;

      while (pos < te) {
        let dr = readVarInt(data, pos); pos = dr.nextPos; tickTime += dr.value;
        ...
        if (mt === 0x90 || mt === 0x80) {
          if (pos + 1 >= te) break;
          const nn = data[pos], vv = data[pos + 1]; pos += 2;
          ...
          if (pos >= te) break;
          const metaType = data[pos++], mlr = readVarInt(data, pos); pos = mlr.nextPos;
  ...
  function readVarInt(data, offset) {
    let vv = 0, p = offset;
    for (let i = 0; i < 4; i++) { if (p >= data.length) break; const b = data[p]; vv = (vv << 7) | (b & 0x7F); p++; if (!(b & 0x80)) break; }
  ...
  function scheduleNext() {
    if (!playerEvents?.length || playerIdx >= playerEvents.length) {
      stopPlayer();
  ...
  function pausePlayer() {
    ...
    playerPaused = true;
    if (isRec && mediaRecorder?.state === 'recording') mediaRecorder.pause();
  ...
  function resumePlayer() {
    ...
    playerPaused = false;
    if (isRec && mediaRecorder?.state === 'paused') mediaRecorder.resume();
    updatePlayUI(true, false);
    scheduleNext();
  }
  ...
  function stopPlayer() {
    ...
    if (isRec) stopRec();
    updatePlayUI(false, false);
    $('playBtn').disabled = !playerEvents.length;
  ...
  $('playBtn').addEventListener('click', () => {
    ...
    else { playerIdx = 0; playerStopped = false; resetMirror(); startPlayer(); }
  });
  $('pauseBtn').addEventListener('click', pausePlayer);
  $('stopBtn').addEventListener('click', stopPlayer);
  $('recBtn').addEventListener('click', () => isRec ? stopRec() : startRec());
  $('saveBtn').addEventListener('click', save);
  $('c1').addEventListener('click', () => { if (lastBlob) { save(); window.open('https://cloudconvert.com/webm-to-mp4', '_blank'); } });
  $('c2').addEventListener('click', () => { if (lastBlob) { save(); window.open('https://convertio.co/webm-mp4/', '_blank'); } });
  ...
  $('bitrate').addEventListener('input', e => { $('bitrateVal').textContent = e.target.value + 'M'; });
